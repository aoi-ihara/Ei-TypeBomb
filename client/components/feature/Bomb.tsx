...
export default function Bomb({ bombStatus, shaking = true }: Props) {
    const bombStyles = [
        {
            body: "#111111",
            fuse: "#7f1d1d",
    ...
    return (
        <svg
            ...
            style={{
                ...
                animation:
                    !shaking || bombShakeDuration === null
                        ? undefined
                        : `bombShake ${bombShakeDuration} ease-in-out infinite`,
            }}
            className={`transition-colors duration-(--duration-etb) ease-etb ${!shaking || bombShakeDuration === null ? "" : "bomb-shake"}`}
        >
            <path
                fill="none"
                stroke={style.fuse}
